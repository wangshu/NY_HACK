...
namespace NYSCQY
{
	public class frmNewDJMC : Form
	{
		...
		private TextBox txtYXCF3;
		private Label label2;
		private TextBox txtYXCF2;
		private Label label4;
		private TextBox txtYXCF1;
		...
		public frmNewDJMC()
		{
			...
			this.SetGridControl();
			this.userManu1.navBarItem8.Appearance.ForeColor = Color.FromArgb(177, 81, 28);
		}
		private void btnSearch_Click(object sender, EventArgs e)
		{
			this.Cursor = Cursors.WaitCursor;
			int num = 0;
			if (this.txtYXCF1.Text.Trim() != "")
			{
				num++;
			}
			if (this.txtYXCF2.Text.Trim() != "")
			{
				num++;
			}
			if (this.txtYXCF3.Text.Trim() != "")
			{
				num++;
			}
			DataTable dataTable = new DataTable();
			dataTable = this.tblResult.Clone();
			for (int i = 0; i < this.tblDJMC.Rows.Count; i++)
			{
				int num2 = 0;
				if (this.txtYXCF1.Text.Trim() != "")
				{
					for (int j = 0; j < this.tblDJMC.Columns.Count; j++)
					{
						if (this.tblDJMC.Rows[i][j].ToString().IndexOf(this.txtYXCF1.Text.Trim()) >= 0)
						{
							num2++;
							break;
						}
				...
				if (this.txtYXCF2.Text.Trim() != "")
				{
					for (int j = 0; j < this.tblDJMC.Columns.Count; j++)
					{
						if (this.tblDJMC.Rows[i][j].ToString().IndexOf(this.txtYXCF2.Text.Trim()) >= 0)
						{
							num2++;
							break;
						}
				...
				if (this.txtYXCF3.Text.Trim() != "")
				{
					for (int j = 0; j < this.tblDJMC.Columns.Count; j++)
					{
						if (this.tblDJMC.Rows[i][j].ToString().IndexOf(this.txtYXCF3.Text.Trim()) >= 0)
						{
							num2++;
							break;
						}
				...
				if (num == num2)
				{
					...
					dataRow["name1"] = this.tblDJMC.Rows[i]["name1"];
					dataRow["name2"] = this.tblDJMC.Rows[i]["name2"];
					dataRow["name3"] = this.tblDJMC.Rows[i]["name3"];
					dataRow["name"] = this.tblDJMC.Rows[i]["name"];
		...
		{
			this.tblResult.Columns.Add("name1");
			this.tblResult.Columns.Add("name2");
			this.tblResult.Columns.Add("name3");
			this.tblResult.Columns.Add("name");
		}
		private void SetGridControl()
		{
			...
			this.gridView1.Columns[0].Caption = "有效成分1";
			this.gridView1.Columns[0].FieldName = "name1";
			this.gridView1.Columns[0].OptionsColumn.AllowEdit = false;
			this.gridView1.Columns[0].OptionsFilter.AllowFilter = false;
			this.gridView1.Columns[0].Width = 190;
			this.gridView1.Columns[1].Caption = "有效成分2";
			this.gridView1.Columns[1].FieldName = "name2";
			this.gridView1.Columns[1].OptionsColumn.AllowEdit = false;
			this.gridView1.Columns[1].OptionsFilter.AllowFilter = false;
			this.gridView1.Columns[1].Width = 190;
			this.gridView1.Columns[2].Caption = "有效成分3";
			this.gridView1.Columns[2].FieldName = "name3";
			this.gridView1.Columns[2].OptionsColumn.AllowEdit = false;
			this.gridView1.Columns[2].OptionsFilter.AllowFilter = false;
			this.gridView1.Columns[2].Width = 190;
			this.gridView1.Columns[3].Caption = "有效成分";
			this.gridView1.Columns[3].FieldName = "name";
		...
		private void frmNewDJMC_Resize(object sender, EventArgs e)
		{
